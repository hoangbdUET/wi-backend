...
async function applyToWell(payload, dbConnection) {
	function createLines(lines, well, _track, dataset) {
		let _lines = [];
		return new Promise((resolve => {
			async.each(lines, (line, next) => {
				delete line.idLine;
				line.idTrack = _track.idTrack;
				checkCurveIsExistedInWell(well, line.curve, dataset, dbConnection).then(curve => {
					if (curve) {
	...
	function createShadings(shadings, well, lines, _track, dataset) {
		return new Promise(resolve => {
			async.each(shadings, function (shading, next) {
				...
				checkCurveIsExistedInWell(well, shading.curve, dataset, dbConnection).then(curve => {
					if (curve) {
	...
	try {
		...
		let dataset = payload.idDataset ? await dbConnection.Dataset.findByPk(payload.idDataset, {include: {model: dbConnection.Curve}}) : null;
		...
		let _lines = await createLines(track.lines, well, _track, dataset);
		// await createAnnotations(track.annotations, _track);
		await createShadings(track.shadings, well, _lines, _track, dataset);
...
function checkCurveIsExistedInWell(well, _curve, dataset, dbConnection) {
	return new Promise(function (resolve) {
		let curve;
		if (dataset) {
			let foundCurve = dataset.curves.find(c => c.name.trim() === _curve.name.trim());
			if (foundCurve) {
				resolve(foundCurve)
			} else {
				resolve(null);
			...
			dbConnection.Dataset.findByPk(_curve.idDataset).then(_dataset => {
				async.each(well.datasets, (d, next) => {
					let foundCurve = d.curves.find(c => c.name.trim() === _curve.name.trim());
					if (foundCurve && d.name.trim() === _dataset.name.trim()) curve = foundCurve;
					next();
				}, function () {
					resolve(curve);
				});

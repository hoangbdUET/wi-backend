...
let editWorkflow = function (data, callback, dbConnection) {
    ...
    dbConnection.Workflow.findByPk(data.idWorkflow).then(w => {
        if (w) {
...
let infoWorkflow = function (data, callback, dbConnection) {
    dbConnection.Workflow.findByPk(data.idWorkflow).then(async w => {
        if (w) {
            w = w.toJSON();
            w.workflowspec = {};
            if (w.idWorkflowSpec) {
                w.workflowspec = await dbConnection.WorkflowSpec.findByPk(w.idWorkflowSpec);
                callback(ResponseJSON(ErrorCodes.SUCCESS, "Successful", w));
            } else {
                callback(ResponseJSON(ErrorCodes.SUCCESS, "Successful", w));
            }
...
let listWorkflow = function (data, callback, dbConnection) {
    ...
    dbConnection.Workflow.findAll({where: {idProject: data.idProject}}).then(w => {
        async.each(w, function (wf, next) {
            wf = wf.toJSON();
            dbConnection.WorkflowSpec.findByPk(wf.idWorkflowSpec).then(ws => {
                wf.workflowSpec = {
                    name: ws.name,
